...
namespace CertificadorWs.Business
{
	public class TimbradoUtils
	{
        ...
        public static string TimbraRetencionString(string comprobante, empresa emp, bool consumeSaldo, bool vaidarUsuario, int idusuario)
		{
            ...
            if(vaidarUsuario)
			TimbradoUtils.ValidarUsuario(comp.Emisor.RFCEmisor,idusuario);
			...
			if (timbre != null && timbre.selloSAT != null && dict.Count == 0)
			{
				...
				if (!TimbradoUtils.GuardaFactura(idusuario,fecha, comp.Emisor.RFCEmisor, rfcReceptor, timbre.UUID, cfdiTimbrado, hash, emp, consumeSaldo, true))
				{
        ...
        public static string TimbraCfdiString(string comprobante, empresa emp, int idusuario)
		{
			...
			if (timbre != null && timbre.SelloSAT != null && dict.Count == 0)
			{
				...
				{
                    if (!TimbradoUtils.GuardaFactura(idusuario,comp.Fecha, comp.Emisor.Rfc, comp.Receptor.Rfc, timbre.UUID, cfdiTimbrado, hash, emp, true, false))
					{
						throw new Exception("Error al abrir el comprobante");
					}
				}
        ...
        public static empresa ValidarUsuario(string rfc, int idusuario)
        {
            NtLinkEmpresa em = new NtLinkEmpresa();
            empresa empresa = em.GetByRfc(rfc);
            empresa result;
            ...
            else
            {
                ...
                var x = NtLinkLogin.ValidateUserPruebas(idusuario);


                if (x.Timbres >= x.TotalTimbres)
                {
                    TimbradoUtils.Logger.Info("Saldo: " + x.Timbres);
                    throw new FaultException("No cuentas con saldo suficiente para timbrar documentos, contacta a tu ejecutivo de ventas");
                }
                result = empresa;
            }
        ...
        public static bool GuardaFactura(int idUsu, string fecha, string rfcEmisor, string rfcReceptor, string uudi, string xml, string hash, empresa emp, bool consumeSaldo, bool retenciones = false)
		{
			...
			if (consumeSaldo)
			{
                ...
                if (tim.IncrementaSaldoPruebas(idUsu))
				{
