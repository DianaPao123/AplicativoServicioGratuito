...
namespace ServicioLocal.Business
{
    public class NtLinkTimbrado : NtLinkBusiness
    {
        ...
        public bool IncrementaSaldoPruebas(int idUsuario)
        {
            try
            {
                using (var db = new NtLinkLocalServiceEntities())
                {
                    var usuario = db.UsuariosPruebas.FirstOrDefault(p => p.ID == idUsuario);
                    if (usuario.Timbres == null)//no tenia pa incrementar el saldo timbrado agregado rgv
                        usuario.Timbres = 1;
                    else
                        usuario.Timbres = usuario.Timbres + 1;

                    usuario.FechaUltimoTimbre = DateTime.Now;
                    db.UsuariosPruebas.ApplyCurrentValues(usuario);
                    //rgv
                    db.SaveChanges();
                    return true;
                }
